...
# path
path_train = "/data/dm/train.typefile"  # 训练文件路径
path_test = "/data/dm/test.typefile"  # 测试文件路径
path_result_out = "model/pro_result.typefile"  # 预测结果文件路径
...
for item in data['TERMINALNO'].unique():
    ...
    night = hour_state[20][0] + hour_state[21][0] + hour_state[22][0] + hour_state[23][0] + \
            hour_state[6][0] + hour_state[0][0] + hour_state[1][0] + hour_state[2][0] \
            + hour_state[3][0] + hour_state[4][0] + hour_state[5][0]
...
model.fit(train1[feature_use].fillna(-1), train1['target'])

# 对测试集进行预测
y_pred = model.predict(test1[feature_use].fillna(-1))
